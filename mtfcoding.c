//This version of the mtf encoder uses hashing to greatly speed up
//the process of checking for repeated words.
//The hashing is not necessary for the decoding process.
//An array is still used for the pile itself.
//Using a linked list for the pile could speed up encoding slightly,
//but it would likely slow down decoding just as much.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "mtfcoding.h"
...
//This struct is used for linked list nodes
//and just to keep track of memory allocated
//to each string in the pile
struct Node{
    char* word;
    int space;
    Node* next;
...
Node* pile; //This serves as the pile or dictionary of words
int pileHeight;
int pileSpace;
...
void init_pile()
{
    pileHeight=0;
    pileSpace=INITSIZE;
    pile=mem_check(NULL,INITSIZE*sizeof(Node));
}
...
//Frees memory used in the hash table
...
//This was used to test how well the hash function was working
...
//Simple hash function that was kind of randomly made 
int hashf(char* str)
{
    int num=1,i;
    for(i=0;str[i]!='\0';i++)
...
{
    pileSpace*=2;
    pile=(Node*)mem_check(pile,pileSpace*sizeof(Node));
}
//Makes new node (duh)
Node* make_new_node(char* word, int space, Node* next)
{
    Node* nn=(Node*)mem_check(NULL, sizeof(Node));
    nn->word=(char*)mem_check(NULL, space*sizeof(char));
    strcpy(nn->word, word);
    nn->space=space;
    nn->next=next;
    return nn;
}
//Searches hash table to check if word has been encountered already
int search_hash_table(char* word)
{
    ...
    if(hashTable[index]==NULL)
        hashTable[index]=make_new_node(word, strlen(word)+1, NULL);
    else
        prev->next=make_new_node(word,strlen(word)+1, NULL);
    return 1;
}
...
//If repeated, the index is found, then the code corresponding
//to its position in the pile is outputted
int encode_word(char *word, FILE* output)
{
    int new,i;
    new=search_hash_table(word);
    if(new)
    {
...
//This function is used to extract words from an mtf file,
//as strtok would not work
void get_word(FILE *f,Node* str)
{
    int start=ftell(f);
    int ch;
    while(1)
    ...
    {
        str->space=length;
        str->word=(char*)mem_check(str->word,str->space);
    }
    fseek(f,start,0);
    fread(str->word,length,sizeof(char),f);
    str->word[length-1]='\0';
    fseek(f,end-1,0);
...
int decode(FILE *input, FILE *output)
{
    ...
    Node* buff=(Node*)mem_check(NULL, sizeof(Node));
    buff->space=0;
